...
export default function CreateProposal({
  ...
  const { isPending, isSuccess, createProposal, reset } = useCreateProposal();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createProposal(formValues, refreshProposals);
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-green-300/80 text-sm mt-1">
                Submitted to the DAO for voting.
              </p>
            </div>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            value={
              formValues.deadline
                ? new Date(formValues.deadline * 1000)
                    .toISOString()
                    .slice(0, 16)
                : ""
            }
        ...
        {!isConnected && (
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 text-yellow-400 text-sm flex items-center gap-2">
            <span className="text-lg">⚠️</span> Connect wallet to create a
            proposal
          </div>
        )}
